...
const EditorContainer = styled.div`
    ...
    .CodeMirror {
        background-color: var(--fallBackGray) !important;
        font-size: 12px;
...
export default ({ style, initialContent, filename, mode, fetchContent, onContentSaved, onModeChanged, onContentChange }: Props) => {
    const [editor, setEditor] = useState<CodeMirror.Editor>();
    const lastFilename = React.useRef<string | undefined>(undefined);
    ...
    useEffect(() => {
        if (editor && filename !== lastFilename.current) {
    ...
    }, [editor, initialContent, filename]);

    useEffect(() => {
        if (!editor || !onContentChange) return;
        const handler = () => onContentChange(editor.getValue());
