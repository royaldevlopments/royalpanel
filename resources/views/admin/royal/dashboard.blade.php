@extends('layouts.royal', ['navbar' => 'components', 'sideEditor' => true])
...
@section('content')
    <form action="{{ route('admin.royal.dashboard') }}" method="POST">
        <div class="header">
        ...
        <div id="components">
            <x-royal.drag-n-drop
                ...
                name="banner"
                label="Egg Banner"
                src="/royal/dashboard/banner.png"
            />
            <x-royal.drag-n-drop
                wide
                name="statCards"
                label="Statistics cards"
                src="/royal/dashboard/statCards.png"
            />
            <x-royal.drag-n-drop
                name="infoAdvanced"
                label="Advanced information card"
                src="/royal/dashboard/infoAdvanced.png"
            />
            <x-royal.drag-n-drop
                wide
                name="graphs"
                label="Graphs"
                src="/royal/dashboard/graphs.png"
            />
            <x-royal.drag-n-drop
                name="info"
                label="Simple information card"
                src="/royal/dashboard/info.png"
            />
            <x-royal.drag-n-drop
                name="SFTP"
                label="SFTP Details"
                src="/royal/dashboard/SFTP.png"
            />
            <x-royal.drag-n-drop
                long
                name="sideGraphs"
                label="Vertical Graphs"
                src="/royal/dashboard/sideGraphs.png"
            />
        </div>

        @php
            $widgets = old('royal:dashboardWidgets', $siteConfiguration['royal']['dashboardWidgets'] ?? []);
        @endphp
        <div id="widget-inputs">
            @foreach ($widgets as $index => $widget)
                <input type="hidden" name="royal:dashboardWidgets[{{ $index }}]" value="{{ $widget }}" placeholder="Widget {{ $index + 1 }}" />
            @endforeach
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                dropzoneItems.forEach(function(item, idx) {
                    ...
                    input.type = 'hidden';
                    input.name = `royal:dashboardWidgets[${idx}]`;
                    input.value = name;
