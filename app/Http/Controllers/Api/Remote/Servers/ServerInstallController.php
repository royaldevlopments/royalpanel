...
namespace RoyalPanel\Http\Controllers\Api\Remote\Servers;
...
use RoyalPanel\Exceptions\Http\HttpForbiddenException;
use RoyalPanel\Repositories\Eloquent\ServerRepository;
use RoyalPanel\Events\Server\Installed as ServerInstalled;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use RoyalPanel\Http\Requests\Api\Remote\InstallationDataRequest;
...
class ServerInstallController extends Controller
{
    ...
    public function store(InstallationDataRequest $request, string $uuid): JsonResponse
    {
        ...
        if ($isInitialInstall && config()->get('royalpanel.email.send_install_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        } elseif (!$isInitialInstall && config()->get('royalpanel.email.send_reinstall_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        }
