...
class Royal extends Command
{
    protected $signature = "royal {action?}";
    ...
    public function handle()
    {
        ...
        if ($action === null) {
            $this->line("\r\n            <title>\r\n            ░█████╗░██████╗░██╗██╗░░██╗\r\n            ██╔══██╗██╔══██╗██║╚██╗██╔╝\r\n            ███████║██████╔╝██║░╚███╔╝░\r\n            ██╔══██║██╔══██╗██║░██╔██╗░\r\n            ██║░░██║██║░░██║██║██╔╝╚██╗\r\n            ╚═╝░░╚═╝╚═╝░░╚═╝╚═╝╚═╝░░╚═╝\r\n\r\n           Thank you for purchasing Royal</title>\r\n\r\n           > php artisan royal (this window)\r\n           > php artisan royal install\r\n           > php artisan royal update\r\n           > php artisan royal uninstall\r\n            ");
        } else {
    ...
    public function installOrUpdate($isUpdate = false)
    {
        ...
        $versions = File::directories("./royal");
        if (empty($versions)) {
            $this->info("No versions found in /royal directory.");
            return;
        }

        $version = basename($this->choice("Select a version:", $versions));
        $this->info("Installing Royal Theme {$version}...");
        ...
        exec("rsync -a {$excludeOption} royal/{$version}/ ./");

        $directoryPath = app_path("Http/Controllers/Admin/Royal");
        File::makeDirectory($directoryPath, 0755, true, true);

        $filesOne = ["RoyalController", "RoyalAdvancedController", "RoyalAnnouncementController", "RoyalColorsController", "RoyalDashboardController", "RoyalSocialController"];
        ...
        $filesTwo = ["RoyalComponentsController", "RoyalLayoutController", "RoyalMailController", "RoyalMetaController", "RoyalStylingController"];
    ...
    private function aa($filename, $version, $directoryPath)
    {
        ...
        $localSource = base_path("royal/" . $version . "/app/Http/Controllers/Admin/Royal/" . $filename . ".php");
    ...
    private function uninstall()
    {
        ...
        $this->command("curl -L https://github.com/royalpanel/panel/releases/latest/download/panel.tar.gz | tar -xzv");
